...
def ensure_data_directory():
    """
    Evita el error de Render cuando /data ya existe.
    Si data es una carpeta, no intenta recrearla.
    """
    ...
    if DATA_DIR.exists():
        ...
        if not DATA_DIR.is_dir():
            raise RuntimeError(
                ...
                "Borrá el archivo 'data' del repositorio y creá una carpeta llamada 'data'."
            )
...
def load_data():
    ...
    try:
        ...
        print(
            "⚠️ economia.json está corrupto. "
            "Se utilizará una estructura nueva."
        )
        ...
        print(
            f"❌ Error cargando economía: {error}"
        )
...
class Economia(commands.Cog):
    ...
    @app_commands.describe(
        usuario="Usuario al que querés agregarle dinero.",
    ...
    async def addmoney(
        ...
        if usuario.bot:
            ...
            return await interaction.response.send_message(
                "❌ No podés modificar el dinero de un bot.",
                ephemeral=True
            )
        ...
        print(
            f"[ECONOMIA ADMIN] "
            f"{interaction.user} agregó "
            f"{cantidad} a {usuario}."
        )
        ...
        embed = discord.Embed(
            ...
            description=(
                ...
                f"a {usuario.mention}.\n\n"
                f"💵 Antes: **{format_money(antes)}**\n"
                f"💵 Ahora: **{format_money(user['cash'])}**"
            ),
            color=PURPLE
        )
        ...
        await self.send_log(
            ...
            "💰 Dinero agregado",
            (
                f"{interaction.user.mention} agregó "
                f"**{format_money(cantidad)}** a "
                f"{usuario.mention}."
            )
        )
    ...
    @app_commands.describe(
        usuario="Usuario al que querés quitarle dinero.",
        cantidad="Cantidad de dinero."
    )
    async def removemoney(
        ...
        user["cash"] -= cantidad_real

        user["stats"]["money_spent"] += cantidad_real

        self.save_data()

        print(
            f"[ECONOMIA ADMIN] "
            f"{interaction.user} quitó "
            f"{cantidad_real} a {usuario}."
        )
        ...
        await self.send_log(
            ...
            (
                ...
                f"**{format_money(cantidad_real)}** a "
                f"{usuario.mention}."
            )
        )
    ...
    @app_commands.command(
        name="setmoney",
        description="Establece el dinero exacto de un usuario."
    )
    @app_commands.describe(
    ...
    async def setmoney(
        ...
        print(
            f"[ECONOMIA ADMIN] "
            f"{interaction.user} estableció "
            f"{usuario} en {cantidad}."
        )

        embed = discord.Embed(
            title="⚙️ Dinero establecido",
            description=(
                f"El saldo de {usuario.mention} "
                f"fue establecido.\n\n"
                f"💵 Antes: **{format_money(antes)}**\n"
                f"💵 Ahora: **{format_money(cantidad)}**"
            ),
            color=PURPLE
        )
        ...
        await self.send_log(
            interaction.guild,
            "⚙️ Dinero establecido",
            (
                f"{interaction.user.mention} estableció el saldo de "
                f"{usuario.mention} en "
                f"**{format_money(cantidad)}**."
            )
        )
    ...
    async def rps(
        ...
        if opcion.value == bot_choice:
            ...
            (
                opcion.value == "piedra"
                and bot_choice == "tijera"
            )
            or
            (
                opcion.value == "papel"
                and bot_choice == "piedra"
            )
            or
            (
                opcion.value == "tijera"
                and bot_choice == "papel"
            )
        ):
